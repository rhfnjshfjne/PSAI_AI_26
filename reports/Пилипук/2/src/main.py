...
def train(X, e, alpha, epochs=20000, tol=1e-6):
    N = X.shape[0]
...
def graph(X, e, weights, T, new_points=None, new_classes=None):
    ...
    if w2 != 0:
        ...
        """Без 0.5 плохо будет. На лекции говорили что надо смещение чтобы оно 
        норм показывало, иначе какашка"""
