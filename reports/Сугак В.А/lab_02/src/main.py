...
def main():
    ...
    for x_r, x, e in zip(X_RAW, X, E):
        print(f"x={x_r} -> класс={model_fixed.predict_class(x, threshold)} (e={int(e)})")

    print("\nАДАПТИВНЫЙ ШАГ")
    print(f"Эпох: {p_adapt} | Финальный Es: {hist_adapt[-1]:.6e}")
    for x_r, x, e in zip(X_RAW, X, E):
        print(f"x={x_r} -> класс={model_adapt.predict_class(x, threshold)} (e={int(e)})")
    ...
    while True:
        ...
        plot_points_and_boundary(model_adapt, X_RAW, E, SCALE, threshold=threshold, user_points=user_points)
        plt.show()
